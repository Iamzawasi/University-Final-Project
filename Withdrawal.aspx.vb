...
Partial Class Withdrawal
    ...
    Protected Sub fntxtb_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ftntxt.TextChanged
        Try
            ...
            ElseIf ftntxt.Text < 0 Then '' if values is negative then 
                ftttxt.Text = "0"
                ftbtxt.Text = "0"    '' bellow fntxtb have negative value multiplied by -1 it will become positive
            ...
            If Me.ftntxt.Text <> "0" Or ftbtxt.Text <> "0" Then
                Me.ftntxt.TabIndex = False
            End If
            If withamnttxtb.Text = transtotal.Text Then
                withbtn.Enabled = True
            Else
                withbtn.Enabled = False
            End If
    ...
    Protected Sub TextBox44_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles otntxtb.TextChanged
        Try


            Dim bundle As Integer
            Dim number As Double
            If otntxtb.Text > 99 Then
                otbtxtb.Text = "0"
                otttxtb.Text = "0"
                bundle = Int(otntxtb.Text / 100) '' it is type is change to int coz it will create problem as we substract number -bundle
                '' if the value is greater then 149 then it will change  it to 200 exp 150/100= 1.5 then it will be change to 2
                otbtxtb.Text = bundle '' it is bundle
                number = (otntxtb.Text / 100)
                otntxtb.Text = Math.Round(100 * (number - Int(bundle))) ''IT IS BUNDLE TEXTBOX
            ...
            ElseIf otntxtb.Text < 0 Then '' if values is negative then 
                otttxtb.Text = "0"
                otbtxtb.Text = "0"    '' bellow fntxtb have negative value multiplied by -1 it will become positive
    ...
    Protected Sub TextBox72_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cntxtb.TextChanged
        Try
            Dim bundle As Integer
            Dim number As Double
    ...
    Protected Sub Depamounttxtb_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles withamnttxtb.TextChanged
        Try
            Dim val As Double
    ...
    Protected Sub TextBox70_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles transtotal.TextChanged
    ...
    Protected Sub withbtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles withbtn.Click
        Try
            Dim cn As String = ConfigurationManager.ConnectionStrings("Dbconnection").ConnectionString
            Dim ccn As New SqlConnection(cn)
            If ccn.State = Data.ConnectionState.Closed Then
            ...
            Else
                CurrencySelectordrplst.Enabled = False
    ...
    Protected Sub DropDownList1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CurrencySelectordrplst.SelectedIndexChanged


        Try
            If CurrencySelectordrplst.SelectedValue = 40 Then ' EURO
            ...
            ElseIf CurrencySelectordrplst.SelectedValue = 30 Then 'PKR
                ...
                fhttxtb.Visible = True
                acntxtb.Enabled = True 'if currency is selected then account txtbox will get active other wise
            ElseIf CurrencySelectordrplst.SelectedValue = 20 Then
                ...
                fhttxtb.Visible = True
                acntxtb.Enabled = True 'if currency is selected then account txtbox will get active other wise
            ElseIf CurrencySelectordrplst.SelectedValue = 10 Then
                ...
                acntxtb.Enabled = True 'if currency is selected then account txtbox will get active other wise
            ElseIf CurrencySelectordrplst.SelectedValue = 1 Then
    ...
    Protected Sub acntxtb_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles acntxtb.TextChanged
        Try
            ...
            Image3.ImageUrl = "~/Uploads/"
            Image4.ImageUrl = "~/Uploads/"
            ...
            If dt.Rows.Count > 0 Then
                ...
                Else
                    ...
                    Image3.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(empimage)
                ...
                Else
                    ...
                    ' Dim ms As New MemoryStream(empimage)
                    'Imagesign.ImageUrl = Image.FromStream(ms)
                    Image4.ImageUrl = "data:image/png;base64," + Convert.ToBase64String(empimage)
            ...
            ElseIf savcuid.Text = currlbl.Text Then
                Remark.Visible = False
                Remark.Text = CurrencySelectordrplst.SelectedValue
                ...
                da = New SqlDataAdapter("select sum(fthousand) as fthousand, sum(othousand) as othousand, sum(fhundred) as fhundred,  sum(ohundred) as ohundred, sum(fifty) as fifty,  sum(twenty) as twenty,  sum(ten) as ten,  sum(five) as five,  sum(two) as two,  sum(one) as one,  sum(cent) as cent from denomination, transactions where  transactions.trans_id=denomination.trans_id and trans_type=0 and ac_id=" & acntxtb.Text.ToString(), ccn)
                dt = New DataTable
                da.Fill(dt)
                ...
                ' Image3.ImageUrl = "~/Uploads/" + Photo
                'Image4.ImageUrl = "~/Uploads/" + Signaure
                '===============================================
            End If
    ...
    Protected Sub CheckBox1_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CheckBox1.CheckedChanged
        If CheckBox1.Checked Then
            savparttxtb.Text = "Cash Paid To " + achname.Text + savparttxtb.Text
        Else
            savparttxtb.Text = ""
        End If
